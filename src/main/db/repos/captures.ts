...
export const captures = {
  list(limit = 50): Capture[] {
    return (
      db()
        .prepare(
          "SELECT * FROM captures WHERE archived_at IS NULL ORDER BY created_at DESC LIMIT ?",
        )
        .all(limit) as Row[]
    ).map(map);
  },
  ...
  archive(id: string): void {
    db()
      .prepare("UPDATE captures SET archived_at = ? WHERE id = ?")
      .run(Date.now(), id);
  },
  latest(): Capture | null {
    const r = db()
      .prepare(
        "SELECT * FROM captures WHERE archived_at IS NULL ORDER BY created_at DESC LIMIT 1",
      )
