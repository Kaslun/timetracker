...
export function FillTab() {
  ...
  const totalLogMinutes = useMemo(
    () => initialSuggestions.filter((s) => picked[s.id]).reduce((acc, s) => acc + s.durationMinutes, 0),
    ...
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      <div style={{ padding: '14px 16px', borderBottom: '1px solid var(--line)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 12 }}>
          <span className="mono ink-3" style={{ fontSize: 10, width: 30 }}>10:30</span>
          ...
          <span className="mono ink-3" style={{ fontSize: 10, width: 30 }}>14:30</span>
        </div>
      </div>
      <div className="scroll" style={{ flex: 1, overflow: 'auto' }}>
        ...
        >
          ...
          <span className="mono ink-3" style={{ fontSize: 10 }}>Drag → timeline</span>
        </div>
        {initialSuggestions.map((s) => (
          ...
          >
            ...
            />
            <span className="mono ink-3" style={{ fontSize: 10, width: 40 }}>{s.at}</span>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div className="mono ink-3" style={{ fontSize: 10, marginTop: 2 }}>{s.meta}</div>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 1 }}>
              ...
              >
                <div
                  style={{ width: `${s.confidence * 100}%`, height: '100%', background: 'var(--accent)' }}
                />
              </div>
            </div>
          </label>
      ...
      >
        ...
        <button className="btn accent" disabled={pickedCount === 0} onClick={() => void onLog()}>
          Log selected
        </button>
      </div>
    </div>
