    ...
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      <div style={{ padding: '12px 14px', borderBottom: '1px solid var(--line)' }}>
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
            ...
            <span className="kbd" style={{ marginLeft: 'auto' }}>Ctrl+Shift+K</span>
          </div>
          <textarea
            value={draft}
      ...
      <div className="scroll" style={{ flex: 1, overflow: 'auto' }}>
        ...
        >
          ...
          {untagged > 0 && (
            <span className="mono ink-3" style={{ fontSize: 10 }}>{untagged} untagged</span>
          )}
        </div>
        {captures.map((c) => (
          <div
          ...
          >
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ display: 'flex', gap: 6, marginTop: 5, alignItems: 'center' }}>
                <span className="mono ink-3" style={{ fontSize: 10 }}>{relativeWhen(c.createdAt, tick)}</span>
                {c.tag ? (
                  <button className="chip" onClick={() => void rpc('capture:tag', { id: c.id, tag: null })}>
                    {c.tag}
                  </button>
                ) : (
                  <select
                    value=""
                    onChange={(e) => void rpc('capture:tag', { id: c.id, tag: e.target.value || null })}
                    className="chip"
                    style={{
                      color: 'var(--accent)',
