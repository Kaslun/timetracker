  ...
  return (
    <div className="attensi window" style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <TitleBar
        ...
        right={
          <>
            <button className="btn ghost icon" title="Search" onClick={onSearch}>
              <Ic.Search s={13} />
            </button>
            <button className="btn ghost icon" title="Settings" onClick={onSettings}>
              <Ic.Settings s={13} />
            </button>
          </>
        }
      />
      <Cockpit />
      <Tabs active={tab} onTab={setTab} inboxCount={inboxCount} fillCount={fillCount} onDump={onDump} />
