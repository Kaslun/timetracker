...
function buildBlocks(entries: EntryRow[], dayStart: number, now: number): Block[] {
  const dayBlocks = entries
    .filter((e) => e.startedAt >= dayStart && e.startedAt < dayStart + 24 * 60 * 60 * 1000)
  ...
  for (const e of dayBlocks) {
    ...
    const eEnd = Math.min(e.endedAt ?? now, dayStart + DAY_END_HOUR * 60 * 60 * 1000);
...
function paletteFor(e: EntryRow): { bg: string; bd: string } {
  switch (e.tag) {
    case 'meet':  return { bg: 'color-mix(in oklab, var(--ink) 6%, var(--surface))', bd: 'var(--line-2)' };
    case 'dev':   return { bg: 'var(--accent-2)', bd: 'var(--accent)' };
    case 'ops':   return { bg: 'var(--surface-2)', bd: 'var(--line-2)' };
    case 'break': return { bg: 'color-mix(in oklab, #6a9d88 12%, var(--surface))', bd: '#6a9d88' };
    default: {
...
export function TimelineTab() {
  ...
  return (
    <div className="scroll" style={{ flex: 1, padding: '14px 16px', overflow: 'auto' }}>
      <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 10 }}>
        <span className="display" style={{ fontSize: 18, fontWeight: 500 }}>Your day</span>
        <span className="mono num ink-3" style={{ fontSize: 11 }}>
          {fmtHM(totalLogged)} logged · <span style={{ color: 'var(--accent)' }}>{fmtHM(totalLoose)} loose</span>
        </span>
      </div>
      <div style={{ position: 'relative', display: 'flex', minHeight: hours.length * HOUR_PX }}>
        <div style={{ width: 40, display: 'flex', flexDirection: 'column' }}>
          {hours.map((h) => (
            <div key={h} className="mono" style={{ height: HOUR_PX, fontSize: 10, color: 'var(--ink-3)', paddingTop: 1 }}>
              {String(h).padStart(2, '0')}:00
            </div>
          ))}
        </div>
        <div style={{ flex: 1, position: 'relative', marginLeft: 4, borderLeft: '1px solid var(--line)' }}>
          {hours.map((h, i) => (
            <div
              key={h}
              style={{
          ...
          {blocks.map((b, i) => {
            if (b.kind === 'gap') {
              return (
                ...
                >
                  <Ic.Plus s={10} />
                  <span style={{ marginLeft: 4 }}>Fill this gap · {b.gapMinutes}m</span>
                </button>
              );
            }
            const e = b.entry!;
              ...
              >
                ...
                {b.heightPx > 36 && (
                  <span className="mono num ink-3" style={{ fontSize: 9 }}>
                    {Math.floor(lenH)}h {String(Math.round((lenH % 1) * 60)).padStart(2, '0')}m
                  </span>
                )}
              </div>
            );
