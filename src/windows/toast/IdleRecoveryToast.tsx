...
import type { IdleNudgePayload } from '@shared/models';

const OPTIONS: { choice: 'discard' | 'meeting' | 'keep' | 'custom'; emoji: string; label: string }[] = [
...
export function IdleRecoveryToast() {
  ...
  const onPick = async (choice: 'discard' | 'meeting' | 'keep' | 'custom'): Promise<void> => {
    if (!payload) return;
  ...
  return (
    <div className="attensi" style={{ height: '100%', padding: 6 }}>
      <div className="card" style={{ width: '100%', height: '100%', padding: '16px 16px 14px', position: 'relative' }}>
        <div
          className="mono num"
          style={{
            position: 'absolute',
        ...
        <div
          className="display"
          style={{ fontSize: 18, fontWeight: 500, letterSpacing: '-0.01em', lineHeight: 1.2 }}
        >
          Welcome back 👋
        </div>
        <div className="ink-2" style={{ fontSize: 12, marginTop: 4, marginBottom: 12, lineHeight: 1.4 }}>
          Your keyboard was quiet for{' '}
          <span style={{ fontWeight: 600, color: 'var(--ink)' }}>{minutes} min</span>. What were you up to?
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
          {OPTIONS.map((o) => (
            <button
              ...
              style={{ justifyContent: 'flex-start', padding: '8px 12px', fontSize: 12 }}
            >
        ...
        <div className="mono ink-3" style={{ fontSize: 10, marginTop: 10, textAlign: 'right' }}>
          Auto-discards in 2h
        </div>
      </div>
    </div>
