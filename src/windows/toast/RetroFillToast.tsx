...
import type { RetroNudgePayload, FillSuggestion } from '@shared/models';
...
export function RetroFillToast() {
  ...
  const visible: FillSuggestion[] = useMemo(() => payload?.suggestions.slice(0, 3) ?? [], [payload]);
  const pickedCount = visible.filter((s) => picked[s.id]).length;
  const pickedMin = visible.filter((s) => picked[s.id]).reduce((a, s) => a + s.durationMinutes, 0);
    ...
    <div className="attensi" style={{ height: '100%', padding: 6 }}>
      <div className="card" style={{ width: '100%', height: '100%', padding: 0, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
        <div style={{ padding: '14px 16px', borderBottom: '1px solid var(--line)' }}>
          <div className="display" style={{ fontSize: 16, fontWeight: 500, letterSpacing: '-0.01em', lineHeight: 1.2 }}>
            A {payload ? formatGap(payload.durationMinutes) : ''} gap just appeared
          </div>
          ...
          {visible.map((s) => (
            ...
            >
              <input
                type="checkbox"
                checked={!!picked[s.id]}
                onChange={(e) => setPicked((m) => ({ ...m, [s.id]: e.target.checked }))}
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div className="mono ink-3" style={{ fontSize: 10 }}>{s.meta}</div>
              </div>
            </label>
          ))}
          {visible.length === 0 && (
            <div className="ink-3" style={{ padding: 20, textAlign: 'center', fontSize: 12 }}>
              Nothing to suggest — gap is yours.
            </div>
          )}
        </div>
        <div style={{ padding: '10px 14px', display: 'flex', gap: 6, alignItems: 'center', borderTop: '1px solid var(--line)' }}>
          <span className="ink-3" style={{ fontSize: 11, flex: 1 }}>
            {pickedCount} picked · {formatGap(pickedMin)}
          </span>
          <button className="btn" onClick={() => void onLater()}>Later</button>
          <button className="btn accent" onClick={() => void onLog()} disabled={pickedCount === 0}>Log</button>
        </div>
      </div>
    </div>
  );
