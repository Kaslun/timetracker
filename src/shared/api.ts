...
interface AttensiBridge {
  windowKind: 'pill' | 'expanded' | 'dashboard' | 'intro' | 'toast' | 'settings' | 'cheatsheet' | 'integration';
...
export async function rpc<C extends ChannelName>(...args: RpcInput<C>): Promise<ChannelOutput<C>> {
  const [channel, input] = args as [C, ChannelInput<C>];
  return (await window.attensi.invoke(channel, input)) as ChannelOutput<C>;
}

export function on<E extends EventName>(event: E, cb: (payload: EventPayload<E>) => void): () => void {
  return window.attensi.on(event, (payload) => cb(payload as EventPayload<E>));
}

export const windowKind    = (): AttensiBridge['windowKind']    => window.attensi.windowKind;
export const toastKind     = (): AttensiBridge['toastKind']     => window.attensi.toastKind;
export const integrationId = (): AttensiBridge['integrationId'] => window.attensi.integrationId;
