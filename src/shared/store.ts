...
export function selectLiveElapsed(state: AppState): number {
  if (!state.current.running || !state.current.startedAt) return state.current.elapsedSec;
...
export function selectLiveTodaySec(state: AppState): number {
  if (!state.current.running || !state.current.startedAt) return state.current.todaySec;
  const liveElapsed = Math.max(0, Math.floor((state.tick - state.current.startedAt) / 1000));
