...
const COLUMN_ORDER = ['date', 'start', 'end', 'duration', 'project', 'client', 'ticket', 'task', 'tag', 'tags', 'notes'] as const;
type ColumnId = typeof COLUMN_ORDER[number];

const PRESET_COLUMNS: Record<Preset, ColumnId[]> = {
  sheets:  ['date', 'start', 'end', 'duration', 'project', 'client', 'ticket', 'task', 'tag', 'notes'],
...
function rangeToDates(range: Range, from?: number, to?: number): { from: number; to: number } {
  const now = Date.now();
  switch (range) {
    ...
    case 'custom': {
      if (from == null || to == null) throw new Error('custom range requires from + to');
      return { from, to };
    }
  }
}
...
export async function exportCsv(
  ...
  const cols = (input.columns.length ? input.columns : presetColumns(input.preset)).filter((c) =>
    COLUMN_ORDER.includes(c as ColumnId)
  ) as ColumnId[];
  ...
  const result = await dialog.showSaveDialog(parent ?? BrowserWindow.getFocusedWindow() ?? new BrowserWindow({ show: false }), {
