...
import type { Entry, EntryRow, EntrySource, CurrentTaskView } from '@shared/models';
...
export const entries = {
  open(): Entry | null {
    const r = db().prepare('SELECT * FROM entries WHERE ended_at IS NULL ORDER BY started_at DESC LIMIT 1').get() as
      | Row
      | undefined;
    return r ? map(r) : null;
  },

  start(input: { taskId: string; startedAt?: number; source?: EntrySource; note?: string | null }): Entry {
    const now = Date.now();
  ...
  pauseAll(at: number = Date.now()): void {
    db().prepare('UPDATE entries SET ended_at = ? WHERE ended_at IS NULL').run(at);
  },
  ...
  update(id: string, patch: Partial<Entry>): void {
    const cur = db().prepare('SELECT * FROM entries WHERE id = ?').get(id) as Row | undefined;
    if (!cur) return;
    const next: Row = {
      ...cur,
  ...
  list(filter: { from?: number; to?: number; taskId?: string } = {}): EntryRow[] {
  ...
  lastClosed(): Entry | null {
    const r = db()
      .prepare('SELECT * FROM entries WHERE ended_at IS NOT NULL ORDER BY ended_at DESC LIMIT 1')
  ...
  currentView(now: number = Date.now()): CurrentTaskView {
    ...
    const row = db().prepare(sql).get(open.taskId) as
      | { id: string; title: string; ticket: string | null; project_name: string; project_color: string }
      | undefined;
