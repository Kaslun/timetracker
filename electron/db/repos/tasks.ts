...
import type { Task, TaskWithProject } from '@shared/models';
...
export const tasks = {
  ...
  listWithStats(now = Date.now()): TaskWithProject[] {
    ...
    `;
    return (db().prepare(sql).all({ now, dayStart }) as JoinedRow[]).map(mapJoined);
  },
  ...
  create(input: { projectId: string; title: string; ticket?: string | null; tag?: string | null; id?: string }): Task {
  ...
  archive(id: string): void {
    db().prepare('UPDATE tasks SET archived_at = ? WHERE id = ?').run(Date.now(), id);
  },
};
