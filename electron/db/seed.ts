...
export function seedIfEmpty(): void {
  const r = db().prepare('SELECT COUNT(*) AS n FROM projects').get() as { n: number };
  if (r.n > 0) return;

  const SEED_PROJECTS = [
    { id: 'prj_mobile',   name: 'Mobile Runtime', color: '#b8633a', ticketPrefix: 'ATT' },
    { id: 'prj_planning', name: 'Planning',       color: '#c9a93a', ticketPrefix: null  },
    { id: 'prj_design',   name: 'Design',         color: '#6a9d88', ticketPrefix: null  },
    { id: 'prj_platform', name: 'Platform',       color: '#5b7ab8', ticketPrefix: 'ATT' },
  ...
  const SEED_TASKS = [
    { id: 'tsk_att412', projectId: 'prj_mobile',   ticket: 'ATT-412', title: 'Fix scenario branching on iOS simulator',  tag: 'dev'   },
    { id: 'tsk_doc88',  projectId: 'prj_planning', ticket: 'DOC-88',  title: 'Draft Q2 planning memo',                   tag: 'write' },
    { id: 'tsk_dsync',  projectId: 'prj_design',   ticket: null,      title: 'Design sync — time tracker v2',            tag: 'meet'  },
    { id: 'tsk_att391', projectId: 'prj_platform', ticket: 'ATT-391', title: 'Refactor auth middleware on learning-api', tag: 'dev'   },
    { id: 'tsk_hire',   projectId: 'prj_hiring',   ticket: null,      title: 'Interview · M. Okafor (staff iOS)',        tag: 'meet'  },
    { id: 'tsk_ops',    projectId: 'prj_ops',      ticket: null,      title: 'Triage #infra inbox',                      tag: 'ops'   },
    { id: 'tsk_att405', projectId: 'prj_mobile',   ticket: 'ATT-405', title: 'Scenario save-load repro',                 tag: 'dev'   },
    { id: 'tsk_att410', projectId: 'prj_mobile',   ticket: 'ATT-410', title: 'Animation bug · iOS 17',                   tag: 'dev'   },
    { id: 'tsk_pr',     projectId: 'prj_platform', ticket: null,      title: 'Code review · 3 PRs',                      tag: 'ops'   },
  ] as const;

  for (const t of SEED_TASKS) {
    tasks.create({ id: t.id, projectId: t.projectId, ticket: t.ticket, title: t.title, tag: t.tag });
  }
  ...
  const sched = (day: number, h: number, lenHours: number, taskId: string, source: 'manual' | 'fill' = 'manual'): void => {
  ...
  const seedToday = (h: number, lenHours: number, taskId: string): void => {
    ...
    entries.insert({ taskId, startedAt: start, endedAt: end, source: 'manual' });
  };
  ...
  const seedCaps = [
    ...
    { text: 'ask ana: are we reusing the old scenario icons?',              tag: '#design',   ago: 12 * minute },
    { text: 'followup — slack thread with eng-mgr re: staffing',            tag: null,        ago: 24 * minute },
    { text: 'write up the reliability findings for friday review',          tag: '#write',    ago: 60 * minute },
    { text: 'schedule 1:1 with sam before sprint end',                      tag: '#admin',    ago: 24 * 60 * minute },
  ];
  for (const c of seedCaps) {
    const cap = captures.create({ text: c.text, tag: c.tag });
    db().prepare('UPDATE captures SET created_at = ? WHERE id = ?').run(now - c.ago, cap.id);
  }
}
